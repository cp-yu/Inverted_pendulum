...
class Ui_Form(QtWidgets.QWidget):
    ...
    def changePID(self):
        ...
        if self.sender()==self.vector_p:
            self.pids.PID_dispPos.setKP(-float(self.sender().text()))
        elif self.sender()==self.vector_i:
            self.pids.PID_dispPos.setKI(-float(self.sender().text()))
        elif self.sender()==self.vector_d:
            self.pids.PID_dispPos.setKD(-float(self.sender().text()))
        # velocity
        elif self.sender()==self.velocity_p:
            self.pids.PID_dispSpd.setKP(-float(self.sender().text()))
        elif self.sender()==self.velocity_i:
            self.pids.PID_dispSpd.setKI(-float(self.sender().text()))
        elif self.sender()==self.velocity_d:
            self.pids.PID_dispSpd.setKD(-float(self.sender().text()))
        # angle  
        elif self.sender()==self.angle_p:
            self.pids.PID_angPos.setKP(-float(self.sender().text()))
        elif self.sender()==self.angle_i:
            self.pids.PID_angPos.setKI(-float(self.sender().text()))
        elif self.sender()==self.angle_d:
            self.pids.PID_angPos.setKD(-float(self.sender().text()))
        # omega
        elif self.sender()==self.omega_p:
            self.pids.PID_angSpd.setKP(-float(self.sender().text()))
        elif self.sender()==self.omega_i:
            self.pids.PID_angSpd.setKI(-float(self.sender().text()))
        elif self.sender()==self.omega_d:
            self.pids.PID_angSpd.setKD(-float(self.sender().text()))
